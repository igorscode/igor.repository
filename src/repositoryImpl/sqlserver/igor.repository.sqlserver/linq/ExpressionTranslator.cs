...
namespace igor.repository.sqlserver.linq
{
    public class ExpressionTranslator
    {
        public string Translate<T>(Expression<Func<T, bool>> predicate)
        {
            ...
            return Translate<T>(node);
        }

        public string Translate<T>(Expression node)
        {

            switch (node.NodeType)
            {
                case ExpressionType.And:
                    throw new Exception("ExpressionType: " + node.NodeType.ToString() + " Not suported.");
                //node = TranslateAnd((BinaryExpression)node);
                //break;
                case ExpressionType.AndAlso:
                    return TranslateAndAlso<T>((BinaryExpression)node);
                case ExpressionType.ArrayIndex:
                    throw new Exception("ExpressionType: " + node.NodeType.ToString() + " Not suported.");
                //node = TranslateBoolean(node);
                //break;
                case ExpressionType.Call:
                    throw new Exception("ExpressionType: " + node.NodeType.ToString() + " Not suported.");
                //node = TranslateMethodCall((MethodCallExpression)node);
                //break;
                case ExpressionType.Constant:
                    throw new Exception("ExpressionType: " + node.NodeType.ToString() + " Not suported.");
                //node = TranslateConstant((ConstantExpression)node);
                //break;
                ...
                case ExpressionType.Not:
                    throw new Exception("ExpressionType: " + node.NodeType.ToString() + " Not suported.");
                //node = TranslateNot((UnaryExpression)node);
                //break;
                case ExpressionType.Or:
                    throw new Exception("ExpressionType: " + node.NodeType.ToString() + " Not suported.");
                //node = TranslateOr((BinaryExpression)node);
                //break;
                case ExpressionType.OrElse:
                    throw new Exception("ExpressionType: " + node.NodeType.ToString() + " Not suported.");
                //node = TranslateOrElse((BinaryExpression)node);
                //break;
        ...
        private string TranslateAndAlso<T>(BinaryExpression node)
        {
            return "(" + Translate<T>(node.Left) + " and " + Translate<T>(node.Right) + ")";
        }
        ...
        private string TranslateComparison(BinaryExpression binaryExpression)
        {
        ...
        private string TranslateComparison(Expression variableExpression, ExpressionType operatorType, ConstantExpression constantExpression)
        {
            string expresion = ((MemberExpression)variableExpression).Member.Name;

            switch (operatorType)
            {
                case ExpressionType.Equal: expresion += " = "; break;
                case ExpressionType.GreaterThan: expresion += " > "; break;
                case ExpressionType.GreaterThanOrEqual: expresion += " >= "; break;
                case ExpressionType.LessThan: expresion += " > "; break;
                case ExpressionType.LessThanOrEqual: expresion += " <= "; break;
                case ExpressionType.NotEqual: expresion = " not " + expresion + " = "; break;
            }

            //expresion += FieldValueSerializerHelper.GetValue(constantExpression.Value, constantExpression.Type);
            expresion += FieldValueSerializerHelper.GetValue(constantExpression.Value, constantExpression.Type);
        ...
        private string processNode<T>(Expression node)
        {
            ...
            var leftVariableExpression = binaryExpression.Left;
            var rightVariableExpression = binaryExpression.Right; // as ConstantExpression;
            var operatorType = binaryExpression.NodeType;

            //TranslateComparison(leftVariableExpression, operatorType, rightVariableExpression);
